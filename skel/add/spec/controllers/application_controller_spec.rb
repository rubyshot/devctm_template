...
describe ApplicationControllerTestController do
  ...
  describe 'require_admin' do
    it 'does nothing if an admin is logged in' do
      controller.should_receive(:current_user).at_least(1).and_return(mock_model(User, :admin? => true))
    ...
    it 'requires an admin if logged in but not admin' do
      controller.should_receive(:current_user).at_least(1).and_return(mock_model(User, :admin? => false))
